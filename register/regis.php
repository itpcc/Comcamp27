...
$(document).ready(function(){
	...
	//Pager moved to statusChangeCallback
	...
	$("#data").on('click', ".get_full[data-id]", function(){
		...
		$.post("/api/index.php/user/full/" + $(this).data("id"), {
			...
			var gender, html;
			$("#getpdf").attr("action", "/api/index.php/user/getpdf/"+userdata.id);
			$("#getpdf input[name=code], #getexcel input[name=code]").val(api_token);
			$("#getpdf input[type=submit], #getexcel input[type=submit]").attr("disabled", false);
			...
			$('#reward').html((typeof result.userdata.computer_reward == "string" && result.userdata.computer_reward.length > 0)?userdata.computer_reward:' - ');

			html = $("#attachment_"+userdata.id).html();
			//console.log("HTML => ", typeof html, '->', html.length);
			if(typeof html !== "string" || html.length <= 0) html = 'ไม่มี';
			$("#attachment").html(html);

			//if student has an image for show
			var imageExist = $(".registree_row[data-id="+id+"] a[data-doc=9]"); //select freestyle first
			if(imageExist.length <= 0)											//if not found
				imageExist = $(".registree_row[data-id="+id+"] a[data-file]");	//choose another
			...
			if(imageExist.length > 0){
				...
				LoadImage("http://graph.facebook.com/"+userdata.fb_id+"/picture?type=normal", $("#image_profile img")[0], function(){
					$("#image_profile").attr("href", "https://www.facebook.com/profile.php?id="+userdata.fb_id);
					$("#image_profile_wait, #image_profile_404, #image_profile_pdf").hide();
					$("#image_profile img").fadeIn();
				});
				...
				LoadImage("http://graph.facebook.com/"+userdata.fbid+"/picture?type=normal", $("#image_profile img")[0], function(){
					$("#image_profile").attr("href", "https://www.facebook.com/profile.php?id="+userdata.fbid);
					$("#image_profile_wait, #image_profile_404, #image_profile_pdf").hide();
					$("#image_profile img").fadeIn();
				});
			}else{
...
function getList(api_token, page) {
	$("#page_now").attr("disabled", true);
	$("#preload").fadeIn('fast');
	$("#data_page, #data").fadeOut();
	console.log('token to getlist ' + api_token + ', page: ' + page) ;
	$.post("/api/index.php/user/fetch_list/" + page + "/"+perPage+"/0", {
		'code': api_token,
		...
		if(result.status == "success"){
			...
			$.each( data, function(i){
				...
				$("#data").append('<tr class="registree_row" data-id="'+id+'"><td>' + id + "</td><td>" + regis_id + '</td><td><a href="javascript: void(0);" class="get_full" data-id="'+id+'">' + first_name + " " + last_name + "</a></td><td>" + nick_name + "</td><td><span id='attachment_" + id + "'></span> </td></tr>");
						...
						$.post("/api/index.php/user/fetch_attachment/" + data[i].id + "/1", {
							...
							if(result.status=="success"){
								...
								if(revision&&revision[0].file_id){
									...
									$("#attachment_" + data[i].id).append("<h6><a href='" + link + "' target='_blank'>ใบสมัครส่วนที่ 1</a></h6>");
								}
							}
						}, 'json');
					}
						...
						$.post("/api/index.php/user/fetch_attachment/" + data[i].id + "/2", {
							...
							if(result.status=="success"){
								...
								if(revision&&revision[0].file_id){
									var file_id = revision[0].file_id;
									link = "/api/user/get_document/" + file_id + "/full";
									$("#attachment_" + data[i].id).append("<h6><a href='" + link + "' target='_blank'>ใบสมัครส่วนที่ 2</a></h6>");
								}
							}
						}, 'json');
					}
						...
						$.post("/api/index.php/user/fetch_attachment/" + data[i].id + "/3", {
							...
							if(result.status=="success"){
								...
								if(revision&&revision[0].file_id){
									var file_id = revision[0].file_id;
									link = "/api/user/get_document/" + file_id + "/full";
									$("#attachment_" + data[i].id).append("<h6><a href='" + link + "' target='_blank'>สำเนาบัตรน้อง</a></h6>");
								}
							}
						}, 'json');
					}
						...
						$.post("/api/index.php/user/fetch_attachment/" + data[i].id + "/5", {
							...
							if(result.status=="success"){
								...
								if(revision&&revision[0].file_id){
									var file_id = revision[0].file_id;
									link = "/api/user/get_document/" + file_id + "/full";
									$("#attachment_" + data[i].id).append("<h6><a href='" + link + "' target='_blank'>ปพ.1</a></h6>");
								}
							}
						}, 'json');
					}
						...
						$.post("/api/index.php/user/fetch_attachment/" + data[i].id + "/4", {
							...
							if(result.status=="success"){
								...
								if(revision&&revision[0].file_id){
									var file_id = revision[0].file_id;
									link = "/api/user/get_document/" + file_id + "/full";
									$("#attachment_" + data[i].id).append("<h6><a href='" + link + "' target='_blank'>ปพ.7</a></h6>");
								}
							}
						}, 'json');
					}
						...
						$.post("/api/index.php/user/fetch_attachment/" + data[i].id + "/7", {
							...
							if(result.status=="success"){
								...
								if(revision&&revision[0].file_id){
									var file_id = revision[0].file_id;
									link = "/api/user/get_document/" + file_id + "/full";
									$("#attachment_" + data[i].id).append("<h6><a href='" + link + "' target='_blank'>ใบขออนุญาต</a></h6>");
								}
							}
						}, 'json');
					}
						...
						$.post("/api/index.php/user/fetch_attachment/" + data[i].id + "/6", {
							...
							if(result.status=="success"){
								...
								if(revision&&revision[0].file_id){
									var file_id = revision[0].file_id;
									link = "/api/user/get_document/" + file_id + "/full";
									$("#attachment_" + data[i].id).append("<h6><a href='" + link + "' target='_blank'>สำเนาบัตร ผปค.</a></h6>");
								}
							}
						}, 'json');
					}
						...
						$.post("/api/index.php/user/fetch_attachment/" + data[i].id + "/8", {
							...
							if(result.status=="success"){
								...
								if(revision&&revision[0].file_id){
									...
									$("#attachment_" + data[i].id).append("<h6><a href='" + link + "' target='_blank' data-doc=\"8\" data-file=\""+file_id+"\" data-extension=\""+revision[0].extension+"\">รูปหน้าตรง</a></h6>");
								}
							}
						}, 'json');
					}
						...
						$.post("/api/index.php/user/fetch_attachment/" + data[i].id + "/9", {
							...
							if(result.status=="success"){
								...
								if(revision&&revision[0].file_id){
									...
									link = "/api/user/get_document/" + file_id + "/full";
									//console.log('id: ' + data[i].id + ', doc_1: ' + doc_1);
									$("#attachment_" + data[i].id).append("<h6><a href='" + link + "' target='_blank' data-doc=\"9\" data-file=\""+file_id+"\" data-extension=\""+revision[0].extension+"\">รูปอิสระ</a></h6>");
								}
							}
						}, 'json');
					}
	...
	if (response.status === 'connected') {
		...
		$.post("/api/index.php/admin/token", {
			...
			if(result.status == "success"){
				...
				$("#getpdf input[name=code], #getexcel input[name=code]").val(api_token);
				$("#getpdf input[type=submit], #getexcel input[type=submit]").attr("disabled", false);
				$.post("/api/index.php/user/count", {
					'code': api_token
				}, function(result){
					cntPage = Math.ceil(result.cnt / perPage);
					...
					$("#next_page").click( function(){
						...
						page = page + 1;
						$("#page_now").val(page);
						//getList(api_token, page); //move to select change
						if(page > 1){
							$("#back_page").fadeIn();
						} else {
							$("#back_page").fadeOut();
						...
						//$("#page_now").html('หน้า ' + page +'/28'); //move to select change
					});
					...
					$("#page_now").change(function(){
						//console.log("Try To Choose => ", $(this).val());
						var pageNow = parseInt($(this).val());
						if(!isNaN(pageNow) && pageNow > 0 && pageNow <= cntPage){
							page = pageNow;
							if(page>1){
								$("#back_page").fadeIn();
							} else {
...
function connectAPI() {
	...
	FB.api('/me', function(response) {
		...
		fb_id = response.id;
		if(typeof window.isAppendFB !== "boolean"){
			$("#fbid").append('ถ้าล็อกอินแล้วยังเข้าไม่ได้ เอารหัสนี้ -> ' + fb_id + ' <- ไปบอกวินด้วยๆ');
			window.isAppendFB = true;
		}
	});
}
			...
			<div class="col-md-10 col-md-offset-1">
				<div class="panel panel-default">
					<div class="panel-body row">
						<div class="col-md-8">น้องที่อัพเอกสารอย่างอื่นแต่ไม่ได้อัพใบคำถาม / อัพใบคำถามมาไม่ครบ บางคนคือน้องเค้าส่งมาทางเมล/ไปรษณีย์แทนนะ</div>
						<div class="col-md-4">
							<form id="getexcel" target="getfile_iframe" method="POST" action="/api/index.php/user/gen_excel/">
								<input type="hidden" name="code" />
								<input type="submit" class="btn btn-primary btn-lg btn-block" value="ดูข้อมูลสรุป" disabled="disabled" onclick="javascript: $('#getexcel').submit();" />
							</form>
						</div>
					</div>
				</div>
			...
			<div class="col-md-10 col-md-offset-1">
				<nav>
					...
					<ul class="pagination">
						...
						<li>
							<select id="page_now"><!-- หน้า 1/28 --></select>
						</li>
						<!-- li>
	...
	<div class="modal-content">
	  ...
	  <div class="modal-body">
		<table class="table">
			...
			<tr>
				<td colspan="3">
					<form id="getpdf" target="getfile_iframe" method="POST">
						<input type="hidden" name="code" />
						<input type="submit" class="btn btn-primary btn-lg btn-block" value="ดูใบสมัคร" disabled="disabled" onclick="javascript: $('#getpdf').submit();" />
					</form>
				</td>
			</tr>
			<tr>
	  ...
	  <div class="modal-footer">
		...
		<iframe name="getfile_iframe" id="getfile_iframe" style="width: 1px; height: 1px; border: 0;" width="1" height="1" frameborder="0" src="/date.php"></iframe>
	  </div>
	</div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div>
