...
function connectAPI() {
  ...
  FB.api('/me', function(response) {
    ...
    if(typeof response.birthday == "string" && Date.parse(response.birthday) !== NaN){
      $( "#birthdate" ).datepicker("setDate", response.birthday);
    }
...
function postWall(){
  FB.api(
    ...
    "POST",
    {
      "message": "ค่ายคอมแคมป์ ครั้งที่ 27 สุดยอดค่ายที่น้องๆ ม.ปลาย จะได้มาสัมผัสประสบการณ์การเรียนวิศวคอมของจริง! เปิดรับสมัครแล้ววันนี้",
  ...
  $.post(/*"/pilot/mute/example/connect-with-js.php"*/"/api/index.php/user/token", {
    ...
    $.post("/api/index.php/user/add", {
      ...
      if(data.status=="fail"){
        ...
        $("#ajax-result").append('<div class="alert alert-warning" role="alert"><b>เกิดความผิดพลาด ทำให้ส่งแบบฟอร์มไม่ได้ </b><br>อาจเกิดจากน้องกรอกข้อมูลไม่ครบ หรือไม่ถูกต้อง ลองกลับไปตรวจสอบข้อมูลดูอีกครั้งนะครับ<br>');
        /*$.each( data.reason, function( key, val ) {
          $("#ajax-result").append('<p>' + key + ': ' + val + '</p>');
        });*/
        $("#ajax-result").append('</div>');
        $("#back").show();
      } else {
          ...
          <div id="regis-form" style="display:none; text-align:left;">
            ...
            <form class="form-horizontal" id="form">
              <span class="page1" ng-show="page==1" ng-init="page=1">
                ...
                <div class="form-group">
                  <label class="col-sm-4 control-label">โรคประจำตัว (เว้นได้)</label>
                  <div class="col-sm-6">
                    <input type="text" class="form-control" id="congenital_disease" ng-model="congenital_disease">
                  </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-4 control-label">อาหาร/สิ่งที่แพ้ (เว้นได้)</label>
                  <div class="col-sm-6">
                ...
                <div class="form-group">
                  <label class="col-sm-4 control-label">โทรศัพท์มือถือ</label>
                  <div class="col-sm-6">
              ...
              <span class="page2" ng-show="page==2">
                <div class="form-group">
                  <label class="col-sm-4 control-label">ชื่อ-นามสกุล ผู้ปกครอง</label>
                  <div class="col-sm-6">
                ...
                <span ng-show="reward==1">
                  <div class="form-group">
                    <label class="col-sm-4 control-label">ผลงาน</label>
                    <div class="col-sm-6">
                ...
                <div class="row">
                  <div class="col-sm-6 col-sm-offset-4">
                    ...
                    <button class="btn btn-success btn-lg" ng-click="page=3">ส่งแบบฟอร์ม</button>
                  </div>
                </div>
              </span>
                  ...
                  <div class="col-sm-8 col-sm-offset-2">
                    ...
                    <div class="getpdf" style="display:none;">
                      ...
                      <p style="text-align:left;">
                        ขั้นตอนต่อไป ให้น้องๆพิมพ์ใบสมัครที่ออกโดยระบบอัตโนมัติ จากนั้นนำไปกรอกข้อมูลและตอบคำถามเพิ่มเติม แล้วจึงส่งใบสมัครมาให้พวกพี่ๆ ตามวิธีที่น้องสะดวกครับ
                      </p>
                ...
                <div class="row">
                  <div class="col-sm-12">
                    <div class="getpdf" style="display:none; text-align:center;">
                      ...
                      <p>น้องๆสามารถส่งใบสมัครได้ทั้งทางไปรษณีย์ / ระบบอัพโหลดเอกสาร / E-mail หรือมาส่งด้วยตนเองที่ภาควิชาฯ</p>
