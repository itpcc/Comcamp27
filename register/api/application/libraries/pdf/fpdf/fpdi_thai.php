	...
	*****************************************************************************************/
	function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link=''){
		...
		if($this->y+$h>$this->PageBreakTrigger && !$this->InHeader && !$this->InFooter && $this->AcceptPageBreak()){
			// Automatic page break : ขึ้นหน้าใหม่อัตโนมัต
			$x = $this->x;
			$ws = $this->ws;
			if($ws>0){
				$this->ws = 0;
		...
		if($txt!==''){
			...
			$this->s_th .= sprintf('BT %.2F %.2F Td (%s) Tj ET',($this->x+$dx)*$k,($this->h-($this->y+.5*$h+.3*$this->FontSize))*$k,$txt2);
	...
	function _checkT($i){   
		...
		if($this->_errorTh($this->array_th[$i])==1)
		{
			...
			if(($this->_errorTh($this->array_th[$i-1])!=2)&&($this->array_th[$i+1]!="ำ"))
			{
				...
				{
					//$pointY=$this->pointY-.01*$this->FontSize*$this->k;
					$pointY=$this->pointY+.1*$this->FontSize*$this->k;
					$this->txt_error=1;
	 			}
			...
			//ตรวจสอบตัวอักษรตัวก่อนหน้านั้นเป็นวรรณยุกต์
			if($this->_errorTh($this->array_th[$i-1])==2)	
			{
				//ถ้าตัวนั้นเป็นไม้เอกหรือไม้จัตวา
				if($this->array_th[$i]=="่"||$this->array_th[$i]=="๋")
				{
					//$pointY=$this->pointY-.001*$this->FontSize*$this->k;
					$pointY=$this->pointY+.3*$this->FontSize*$this->k;
					$this->txt_error=1;
				}
				//ถ้าตัวนั้นเป็นไม้โทหรือไม้ตรี
				elseif($this->array_th[$i]=='้'||$this->array_th[$i]=='๊')
			...
			if($this->_errorTh($this->array_th[$i-1])==3)		
			{
				...
				if($this->array_th[$i]=="่"||$this->array_th[$i]=="๋")
				{
					$pointX=$this->pointX-.3*$this->GetStringWidth($this->array_th[$i-1])*$this->k;
					//$pointX=$this->pointX+.10*$this->GetStringWidth($this->array_th[$i-1])*$this->k;
					$this->txt_error=1;
				}
				//ถ้าตัวนั้นเป็นไม้โทหรือไม้ตรี
				elseif($this->array_th[$i]=='้'||$this->array_th[$i]=='๊')
			...
			{					
				...
				{
					$pointX=$this->pointX-.1*$this->GetStringWidth($this->array_th[$i-2])*$this->k;						
					$this->txt_error=1;
				}
			}
		}
		...
		//ตรวจสอบการแสดงผลของตัวอักษรสระบน
